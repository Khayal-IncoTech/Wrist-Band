...
void app_main(void)
{
    ...
    // Adaptive LED control: keep ir_raw in the [target_lo, target_hi] band by
    // adjusting the IR LED current. Compensates for dark/light skin, hair, and
    // wrist thickness without any user intervention.
    uint8_t ir_led_current = 0x7F;       // matches the value set in max30102_init()
    uint64_t ir_sum = 0;
    int ir_sum_count = 0;
    const int led_check_interval = 200;  // 2 s at 100 Hz
    const uint32_t led_target_lo = 30000;
    const uint32_t led_target_hi = 200000;
    const uint8_t led_step = 0x10;
    const uint8_t led_min = 0x20;        // ~6 mA, lower than this signal is too weak
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (ir_sum_count >= led_check_interval && filter_primed) {
                ...
                if (new_current != ir_led_current) {
                    ...
                    // DC level will shift after a LED change; re-prime filter
                    // and restart warmup so the step doesn't poison detection.
                    filter_primed = 0;
                }
            }
        }
